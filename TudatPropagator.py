...
def propagate_orbit(Xo, tvec, state_params, bodies=None):
    '''
    ...
    if 'Jupyter' in bodies_to_create:
        
        # Gravity
        acceleration_settings_setup['Jupyter'] = [propagation_setup.acceleration.point_mass_gravity()]

    if 'Mars' in bodies_to_create:
        
        # Gravity
        acceleration_settings_setup['Mars'] = [propagation_setup.acceleration.point_mass_gravity()]

    if 'Venus' in bodies_to_create:
        
        # Gravity
        acceleration_settings_setup['Venus'] = [propagation_setup.acceleration.point_mass_gravity()]

    if 'Saturn' in bodies_to_create:
        
        # Gravity
        acceleration_settings_setup['Saturn'] = [propagation_setup.acceleration.point_mass_gravity()]
...
def propagate_state_and_covar(Xo, Po, tvec, state_params, bodies=None, alpha=1e-4, backward=False):
    '''
    This function propagates an orbit using physical parameters provided in 
    ...
    if 'Earth' in bodies_to_create:
        ...
        if sph_deg == 0 and sph_ord == 0:
            ...
            acceleration_settings_setup['Earth'] = [propagation_setup.acceleration.spherical_harmonic_gravity(10, 10)]
    ...
    if backward:
        termination_condition = propagation_setup.propagator.non_sequential_termination(
            propagation_setup.propagator.time_termination(796787770.0, terminate_exactly_on_final_condition=True),
            propagation_setup.propagator.time_termination(796780800.0, terminate_exactly_on_final_condition=True)
        )
    else:
        # Create termination settings
        termination_condition = propagation_setup.propagator.time_termination(
